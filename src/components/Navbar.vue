    ...
    <header class="navbar dark:bg-backgroundDark dark:border-b dark:border-primary/10 sticky top-0 z-10 shadow-sm ">
        ...
        <section class="flex-1">
            <div class="form-control">
                ...
                dark:[--webkit-backdrop-filter(10)] dark:border-white/10" />
            </div>
        </section>
        <section class="flex gap-6">
            <!-- Theme toggle Button -->
             <button @click="$emit('toggle-theme')" class="btn btn-circle btn-ghost dark:bg-white/5 dark:backdrop-blur-md 
             dark:[--webkit-backdrop-filter(10)] dark:border-white/10">
                <Icon v-if="isDark" icon="line-md:moon-alt-to-sunny-outline-loop-transition" class="inline-block w-8 h-8" />
                <Icon v-else icon="line-md:sunny-outline-to-moon-alt-loop-transition" class="inline-block w-8 h-8" />
            </button>
